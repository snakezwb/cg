...
var Util = (() => {
   ...
   * 处理数组元素
   */
  Util.map = (arr, callback) =>
    nativeMap.call(arr, callback);
  ...
  Util.map = Util._splitObject((isArray, list, arr, callback) => {
    ...
    for (; i < len; i++) {
      key = isArray ? i : list[i];
      value = arr[key];
      ((result = callback.call(value, value, key, arr)) != null) && (res.push(result));
    }
   ...
   * 将二维数组转成一维数组
   */
  Util.parseArrayToSimple = (arr) =>
    arr.length ? nativeConcat.apply([], arr) : arr;

  /**
   * 将连接符写法转成小驼峰写法
   */
  Util.camelize = (str) =>
    str.replace(camelizeRE, (_, chr) => chr ? chr.toUpperCase() : '')
